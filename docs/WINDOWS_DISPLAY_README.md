# Windows 显示器控制功能使用说明（WiFi 备用方案）

## 功能概述

**注意**：这是**备用方案**，推荐使用 USB 连接方案（见 [USB_DISPLAY_README.md](USB_DISPLAY_README.md)）。

Android 应用可以通过 WiFi 网络远程控制 Windows 电脑的显示器显示模式：
- 仅第一屏（笔记本内置屏幕）
- 仅第二屏（外接显示器）
- 扩展模式（双屏同时使用）
- 复制模式（双屏显示相同内容）
...
```
Android 手机 → WiFi 网络 → HTTP RPC → Windows 服务端 → DisplaySwitch.exe → 切换显示器
```
